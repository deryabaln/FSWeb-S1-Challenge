...
function indekseGoreFenomen(newArray,index) {
  return index + '. indekste bulunan fenomen: ' + newArray[index].profile
}
...
function profilListesi(newArray) {
  ...
  for(let i=0; i< newArray.length; i++){
    yeniDizi.push(newArray[i]);
     ...
     result.push(yeniDizi[i].profile)
  }
  return result
} 
...
function fenomenSil(newArray, index){
  let yeniDizi = [];
  for(let i=0; i< newArray.length; i++){
    yeniDizi.push(newArray[i]);
  }
  return  yeniDizi.slice(index+1)
}
...
function fenomenEkle(newArray, numara, profil, takipçiler,gönderi, sosyalmedya) {
  let yeniFenomen = {'number': numara, 'profile': profil, 'followers': takipçiler, 'posts': gönderi, 'platform': sosyalmedya};
	let yeniDizi = newArray.push(yeniFenomen);
	for(let i= 0; i<yeniDizi.length; i++){
		 console.log(fenomenEkle(newArray, numara, profil, takipçiler,gönderi, sosyalmedya))
	}
	return newArray
}
...
function enFenomenler(newArray) {
  ...
  for(let i = 0; i< newArray.length; i++){
    if(newArray[i].followers >100000000){
      result.push(newArray[i].profile)
    }
  }
  return result
}
...
function fenomenGonderimSayisi(newArray, fenomenAdi){
  let gonderi = 0
  for(let i=0; i<newArray.length; i++){
    if(fenomenAdi == newArray[i].profile){
      gonderi = newArray[i].posts
    }
   } 
  return gonderi
}
...
function platformaGoreCokGonderiYapanFenomen(newArray, sosyalMedya){
  let gonderi = []
  let tiktokİcerenler = []
  let enFazlaPost = 0
  for(let i=0; i<newArray.length; i++){
    if(sosyalMedya == newArray[i].platform){
      gonderi.push(newArray[i].posts)
      tiktokİcerenler.push(newArray[i])
    } 
  }
  return tiktokİcerenler[gonderi.indexOf(Math.max.apply(null, gonderi))].profile
} 
